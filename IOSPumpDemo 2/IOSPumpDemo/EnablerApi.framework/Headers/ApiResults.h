...
typedef NS_ENUM( NSUInteger, ENBApiResult)
{
    ...
    /// 
    /// A invalid authorisation limit has been specified.
    /// 
    ...
    /// 
    /// The connection went offline while communicationg with Enabler Pump Server.
    /// 
    ENBApiResultErrorWithPumpServer,                 // 29
    /// 
    /// Information result; not an error.
    /// It indicates a successful logon, but that one or more pumps that this terminal had reserved previously have been unreserved.
    /// 
    ENBApiResultReservesRemoved,                 // 30
    /// 
    /// Information result; not an error.
    /// It indicates a successful logon, but that some locks this terminal had previously were removed.
    /// 
    ...
    /// 
    /// Information result; not an error.
    /// It indicates a successful logon, but that some locks or reserves this terminal had have been removed.
    /// 
    ...
    /// 
    /// The pump cannot be authorised because the configured maximum number of deliveries are already in progress (see Enabler configuration).
    /// 
    ...
    /// 
    /// Attendant logoff failed because no attendant is logged onto the pump.
    /// 
    ENBApiResultPumpNotLoggedOn,               // 40
    /// 
    /// Attendant logoff failed because the Attendant parameter did not match the Attendant currently logged onto the pump.
    /// 
    ...
    /// 
    /// The authorisation limit is too small and cannot be set at the pump.
    /// 
    ENBApiResultLimitationTooSmall,             // 54
    /// 
    /// The authorisation limit is above the allocation limit defined for the Grade  - authorisation failed.
    /// 
    ...
    /// 
    /// Attendant logoff not permitted because the attendant still has some transactions that have been completed but not cleared
    /// 
    ...
    /// 
    /// Indicates that the Attendant may not logon because they have a previous period that has not yet been reconciled.
    /// 
    ...
    /// 
    /// The pump cannot be authorised because it is disabled.
    ...
    /// 
    /// All the hoses selected in the authorisation request are disabled or are linked to a disabled grade or tank object.
    /// 
    ENBApiResultAllSelectedHosesBlocked,	      // 75
    /// 
    /// The pump cannot be authorised because either:
    /// - the calling hose id is known and is disabled.
    /// - the calling hose id is unknown and one or more hoses on the pump are disabled. The authorisation must be declined to prevent the possibility of a disabled hose delivering fuel.
    /// Note this error can only be returned if the pump is already calling when the authorise attempt is made.
    /// 
    ...
    /// 
    /// This pump was never authorised or a delivery has already started.
    /// 
    ...
    /// 
    /// New authorisations are not allowed when system is running on UPS battery.
    /// 
    ENBApiResultSystemOnUpsBattery,                     // 97
    
    /// 
    /// DataBase queue is too large. (Enabler Embedded Only). This stops Authporisations until queue is reduced.
