 ...
 */
@protocol EnablerSessionDelegate
@optional
/*!
 ...
 *  respond with the SendEventAcknowlege method to achnowledge the receiot of the events. Only the last event of a block needs to be acknowleged.
 *
...
/*!
 *  This delagate is fired when a heartbeat has been received. If user is receiving these delegates then it is expected to  send a heartbeat
 *  in response. if dekagate is not reponded to then the session will automatically generate a response.
 *
...
/*!
 *  Create a connection to the Pump Server at serverAddress and login with the passed pararmeters
 *
 *  @param serverAddress  IP address or Host name of server where Pump Server is running.
 *  @param withTerminalID The terminal ID for login
...
/*!
 *  Send a XML request of type and wait for response sychronously or via a completion handler.
 *
...
*
*  @param completeXML <#completeXML description#>
*/
...
/*!
 *  Send an event acknownledgement.
 *
 ...
 *                          All proceding events up to the specified event will be marked as acknowledged.
 */
- (void) SendEventAcknowledege:(int)lastEventNumber;

/*!
 *  Used to add a reference to a class instance responding to the id<EnablerSessionDelegate> protocoll so that the class
 *  will receive the fired delgates.
 *
 *  @param delegate id responding to id<EnablerSessionDelegate>
 */
-(void)addDelegate:(id<EnablerSessionDelegate>) delegate;

/*!
 *  Used to remove a class instance form the delgate list, see addDelegate
 *
 *  @param delegate delegate id that been previously added to delegatee
 */
-(void)removeDelegate:(id<EnablerSessionDelegate>) delegate;
